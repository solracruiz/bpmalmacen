...
namespace bpmalmacen
{
    public partial class Det_Entradas : Form
    {
        ...
        int validar_ent()
        {
            ...
            if (txtfecha.Text != "" && cbtipo.Text != "" && cbalmacen.Text != "" && cbrecibio.Text != "" && txtimporte.Text == P.ToString())
            { return 1; }
            return 0;
        }
            ...
            try
            {
                if (validar_ent() == 0){ return; }
                    ...
                    conn.Executa("insert into entradas (tipo,fechaentrada,numfactura,fechafactura,costofactura," +
                        "idproveedor,idarea,idrecibio,idalmacen,estatus) values('" + 
        ...
        private void Grid_Det_Ent_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            for (int fila = 0; fila < Grid_Det_Ent.Rows.Count - 1; fila++)
            {
                ...
                str= "insert into kardex (idtabla,idarticulo,entrada,salida,precio,costo,fecha)" +
                   "values(" + ID_ENT + "," +
                    ...
                    Grid_Det_Ent.Rows[fila].Cells[3].Value.ToString() + "," +
                    Grid_Det_Ent.Rows[fila].Cells[4].Value.ToString() + ",'" +
                    txtfecha.Value.ToString("yyyy-MM-ddd") + "')";

                //conn.Executa("update catarticulos set existencia=existencia+" + Grid_Det_Ent.Rows[fila].Cells[2].Value.ToString());
